...
export class TuiLayout {
  ...
  public renderHeader(from: Address, to: Address, level: DetailLevel): void {
    ...
    const header = `${AnsiColors.bold}httpSandwich${AnsiColors.reset} ${from.toString()} ${AnsiColors.dim}→${AnsiColors.reset} 🥪 ${AnsiColors.dim}→${AnsiColors.reset} ${to.toString()}  ${AnsiColors.dim}[${level.toString()}]${AnsiColors.reset}`;
